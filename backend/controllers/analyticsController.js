...
/**
 * @desc    Get dashboard analytics for a School Admin
...
const getDashboardAnalytics = asyncHandler(async (req, res) => {
  ...
  const totalIssued = await Transaction.countDocuments({
    school: schoolId,
    status: { $in: ['Issued', 'Overdue'] } // Get all non-returned
  });
  ...
  res.status(200).json({
    coreStats: {
      ...
      totalStudents,
      totalIssued,
      totalStaff, // We also need this for the overview
    },
    booksBySubject,
    recentTransactions,
  });
...
// (These are helper functions for the invoice generation)
...
// --- THIS IS THE FIX ---
// The export block was missing from your file
export { 
