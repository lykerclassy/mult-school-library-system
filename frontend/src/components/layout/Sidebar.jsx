...
const NavItem = ({ link }) => {
  ...
  return (
    <NavLink to={link.path} className={({ isActive }) => twMerge('flex items-center space-x-3 p-2 rounded-md transition-colors text-gray-300 hover:bg-gray-700 hover:text-white', isActive && 'bg-primary text-white', !isOpen && 'justify-center')} >
      <link.icon className="w-5 h-5 flex-shrink-0" />
      <span className={twMerge('transition-all duration-200', !isOpen && 'w-0 opacity-0')}>{link.name}</span>
    </NavLink>
  );
};
...
const Sidebar = () => {
  ...
  const navLinks = {
    Developer: [
      ...
      { name: 'Global Users', icon: Users, path: '/dev/users' }, // <-- ADDED
      { name: 'Global Resources', icon: Globe, path: '/dev/resources' }, // <-- ADDED
  ...
  return (
    <aside className={twMerge('h-screen bg-gray-800 text-white flex flex-col transition-all duration-300 z-40 fixed md:relative', isOpen ? 'w-64' : 'w-20', !isOpen && 'md:w-20', !isOpen && '-translate-x-full md:translate-x-0')} >
      <div className={twMerge('h-16 flex items-center justify-between border-b border-gray-700 px-4', !isOpen && 'px-0 justify-center')} >
        <span className={twMerge('text-2xl font-bold transition-all', !isOpen && 'w-0 opacity-0')}>LMS Portal</span>
        <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-md hover:bg-gray-700 md:hidden"><ChevronsLeft className="w-6 h-6" /></button>
      </div>
      ...
      <div className="p-4 border-t border-gray-700">
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="hidden md:flex items-center justify-center w-full p-2 mt-2 rounded-md hover:bg-gray-700">
